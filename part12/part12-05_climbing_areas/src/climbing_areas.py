...
def sort_by_number_of_routes(areas: list):
    def routes_order(area: ClimbingArea):
        return area.routes()
    
    return sorted(areas, key=routes_order)

def sort_by_most_difficult(areas: list):
    def difficult_order(area: ClimbingArea):
        return area.hardest_route().grade

    return sorted(areas, key=difficult_order, reverse=True)
